...
module.exports = {
  name: "play",
  description: "Tocar audio do Youtube",
  async execute(message, args) {
    const { channel } = message.member.voice;

    if (!args.length) return message.reply("Use: "+PREFIX+"play <URL do Youtube | Nome do Video>").catch(console.error);
    if (!channel) return message.reply("Você precisa estar em um canal de voz primeiro!").catch(console.error);

    const permissions = channel.permissionsFor(message.client.user);
    if (!permissions.has("CONNECT"))
      return message.reply("Não é possível conectar ao canal de voz, preciso de permissões");
